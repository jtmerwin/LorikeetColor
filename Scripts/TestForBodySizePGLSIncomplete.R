runPGLSBodySizeToColoranalysis <- function(colPCSelect = 1)
{
  ...
  byPatchPGLS <- function(patches = NULL, colpcVal = 1, cleanedDataset = cleanedData, climData, brightnessSelect = brightnessOn)
  {
    ...
    caperModel <- caper::pgls(formula = V1 ~ PC1*PC2*PC3, data = na.omit(caperCompare))
    print("pglsRun")
    return(caperModel)
  }
  ...
  if(retHist == T){(return(as.numeric(paste(returnTab))))}
